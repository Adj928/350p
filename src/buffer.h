...
typedef struct BufferManager {
  ...
  /**
   * An array of .frame_count status objects.  Each entry .status[i] corresponds to .headers[i].
   */
  FrameHeader *headers;
  
  /**
   * An array of .frame_count frame buffers.  Each entry .headers[i] corresponds to .status[i]
   */
  Frame *frames;
